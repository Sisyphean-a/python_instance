...
```python
# 使用plot()进行做图，这里是两组数据，所以y轴使用的是列表
df.plot(y=["平均价","最高价"],	# 这里没有指明x轴，默认调用index为x轴
    kind="bar",					# 设置使用柱状图
    color=["red","orange"],		# 设置颜色，和y相似，使用列表设置两个颜色
    title="US油价",				
    rot=60)						# 设置x轴标签倾斜角度

# plot()无法更加精细，使用matplotlib进行更一步修改
plt.title("US油价",fontsize=18,fontweight="bold")

# 可以发现，plt可以直接在plot的基础上继续做图，这是因为plot本身使用的就是plt库
